...
정책 'standard' | 목표 18% (sell_price 기준) | VAT general
원가 62,000원 + 매입배송 0 + 발송배송 3,000 + 부자재 500

마켓            판매가        수수료       VAT      순이익    마진율
------------------------------------------------------------------
naver          90,900       7,036     1,669    16,695    18.4%
coupang        97,900      12,531     1,806    18,063    18.5%
```
...
* `margin_basis`: `sell_price`(판매가 기준) / `cost`(원가 마크업)
...
- [ ] `pricing.policies.*.fees` 를 **실제 정산 내역서** 기준으로 교정
      (기본값은 예시일 뿐입니다 — 카테고리·계약별로 다릅니다)
