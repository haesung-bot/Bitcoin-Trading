...
#
# ⚠️ 여기에 후보를 빠뜨리면 워크포워드가 그 설정을 '고를 기회조차' 없다.
#    그래놓고 나중에 별도로 튜닝한 설정의 성과를 비교하면 불공정하다
#    (별도 튜닝은 검증 구간과 겹치는 데이터를 봤을 수 있기 때문).
#    그래서 세부 파라미터(sl_buffer_atr, ltf_swing)도 여기 포함시킨다.
#
# 타임프레임은 4h/1h로 고정했다. 1단계 탐색에서 학습 구간 기준 상위권을
# 4h/1h가 전부 차지했고, 15분봉을 넣으면 구간당 계산량이 3배로 늘어난다.
WF_GRID = {
    "tf_pair": [("4h", "1h")],
    "min_rr": [1.5, 2.0, 3.0],
    "sl_mode": ["swing", "wider"],
