...
eq("기본은 꺼짐(검증 결과 반영)", app.quiet_on_var.get(), my.QUIET_ON_DEFAULT)
check("기본값이 꺼짐인지", my.QUIET_ON_DEFAULT is False)
...
eq("체크 꺼짐이면 정지 비활성", seen.get("quiet", (None,))[0], -1)
eq("한국 시간 오프셋은 반영", seen.get("quiet", (0, 0, None))[2], my.QUIET_TZ)
eq("정지 손절차수도 0", seen.get("quiet", (0, 0, 0, None))[3], 0)
...
check("설정 파일에 정지 설정 저장", saved.get("quiet_start") == str(my.QUIET_START_DEFAULT)
      and saved.get("quiet_on") is my.QUIET_ON_DEFAULT)
eq("재시작 후 정지 시각 복원", app2.quiet_start_var.get(), str(my.QUIET_START_DEFAULT))
eq("재시작 후 꺼짐 복원", app2.quiet_on_var.get(), my.QUIET_ON_DEFAULT)
